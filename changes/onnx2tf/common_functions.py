...
def equalize_tensors_dim(tensors):
    """
    There are some layers, that can get inputs with different shapes (e.g. Mul).
    This func leads layers to the same shape (one with the largest number of dimensions). 
    It will equalize objects with 'values' attr, therefore the maximum number of 
    gs.Variable tensors is only one and it has the maximum number of dimensions 

    NWH -> NCWH
    """
    max_len = len(max(tensors, key=lambda a: len(a.shape)).shape)
    min_len = len(min(tensors, key=lambda a: len(a.shape)).shape)
    assert max_len - min_len <= 1

    if max_len - min_len != 0:
        for i in range(len(tensors)):
            inp = tensors[i]
            if len(inp.shape) != max_len:
                if hasattr(inp, 'values'):
                    inp.values = inp.values.reshape([inp.shape[0], 1, *inp.shape[1:]])
                else:
                    raise Exception(
                        f"""Input must have 'values' attr, or must have the largest len(shape), but 
                        tensors[{i}] ('{inp.name}') with shape {inp.shape} is not the largest and has no 'values' attr"""
                    )
